...
void ColorPanel::step() {
    if (inputs[RED_INPUT].active) {
        float in_value = clampf(inputs[RED_INPUT].value, in_min[inputRange], in_max[inputRange]);
        red = rescalef(in_value, in_min[inputRange], in_max[inputRange], 0.0f, 1.0f);
    }
    if (inputs[GREEN_INPUT].active) {
        float in_value = clampf(inputs[GREEN_INPUT].value, in_min[inputRange], in_max[inputRange]);
        green = rescalef(in_value, in_min[inputRange], in_max[inputRange], 0.0f, 1.0f);
    }
    if (inputs[BLUE_INPUT].active) {
        float in_value = clampf(inputs[BLUE_INPUT].value, in_min[inputRange], in_max[inputRange]);
        blue = rescalef(in_value, in_min[inputRange], in_max[inputRange], 0.0f, 1.0f);
    }
}
...
ColorPanelWidget::ColorPanelWidget() {
    ColorPanel *module = new ColorPanel();;
    setModule(module);
    ...
    {
        panel = new Panel();
        panel->backgroundColor = nvgRGB(128, 128, 128);
    ...
    {
        color_frame = new ColorFrame();
		color_frame->box.size = Vec(box.size.x, RACK_GRID_HEIGHT);
        color_frame->box.pos = Vec(0, 0);
        color_frame->module = module;
        addChild(color_frame);
