...
// 0v = C4 = 261.626f
#define VCO_BASELINE_VOLTAGE 0.0f
#define VCO_BASELINE_FREQ 261.626f
#define VCO_BASELINE_NOTE_OCTAVE_OFFSET 4.0f

//float VCO_BASELINE_VOLTAGE = 4.75f;
//float VCO_BASELINE_FREQ = 440.0f;


#define A440_MIDI_NUMBER 69

// bogaudio lfo, defaults to 0v == 16.35f
// float LFO_BASELINE_FREQ = 16.35f;

// https://vcvrack.com/manual/VoltageStandards.html#pitch-and-frequencies
#define LFO_BASELINE_VOLTAGE 0.0f
#define LFO_BASELINE_FREQ 2.0f
...
//   261.626f == C4 == 0v
inline float freq_to_cv(float freq) {
...
inline float lfo_freq_to_cv(float lfo_freq) {
...
inline float cv_to_freq(float volts) {
...
inline float lfo_cv_to_freq(float volts) {
...
inline float lfo_cv_to_bpm(float volts) {
    return lfo_cv_to_freq(volts) * 60.0;
}

// can return negative
inline double volts_of_nearest_note(float volts) {
    double res = round( (volts * 12.0) )  / 12.0;
    return res;
}

inline int volts_to_note(float volts) {
...
inline int volts_to_octave(float volts) {
...
inline float volts_to_note_cents(float volts) {
...
inline int volts_to_midi(float volts) {
